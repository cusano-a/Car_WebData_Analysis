...
pattern_manufacturer = re.compile(
    r'\"makeId\":(.*?),\"modelOrModelLineId\":(.*?),\"make\":"?(.*?)"?,\"model\":"?(.*?)"?,\"modelVersionInput\":"?(.*?)"?,'
)
pattern_vendor_location = re.compile(
...
def get_car_URLs(country, numpages, offsetpag, db=False):
    ...
    for page in range(1 + offsetpag, 1 + offsetpag + numpages):
        try:
            url = (
                used_cars_website
                + "/lst?sort=age&desc=1&ustate=N%2CU&size=20&page="
                + str(page)
                + "&cy="
                + countries[country]
                + "&atype=C"
            ...
            soup = BeautifulSoup(
                urllib.request.urlopen(url).read(), "lxml", parse_only=only_a_tags
            )
        except Exception as e:
            if db:
                print(f"Error page {page}: " + str(e) + " " * 50, end="\r")
            pass
...
def get_car_dict(URL, db=False):
    ...
    # Manufacturer data
    manufacturer_res = re.findall(
        pattern_manufacturer, car.find(id="__NEXT_DATA__").text
    )
    if manufacturer_res:
        (
            car_dict["makeId"],
            car_dict["modelOrModelLineId"],
            car_dict["maker"],
            car_dict["model"],
            car_dict["modelVersionInput"],
        ) = manufacturer_res[0]
    else:
        (
            car_dict["makeId"],
    ...
    car_dict["dealer"] = (
        car.find(
            "div",
            attrs={"class": "cldt-vendor-contact-box", "data-vendor-type": "dealer"},
        )
        != None
    )
    car_dict["privateSeller"] = (
        car.find(
            "div",
            attrs={
                "class": "cldt-vendor-contact-box",
                "data-vendor-type": "privateseller",
            },
        )
        != None
    )
    ...
    location_res = re.findall(
        pattern_vendor_location, car.find(id="__NEXT_DATA__").text
    )
    if location_res:
        (
            car_dict["countryCode"],
            car_dict["zip"],
            car_dict["city"],
            car_dict["street"],
        ) = location_res[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-c",
        dest="country",
        help="Country where to search cars",
        default="Italy",
    ...
    for ii, URL in enumerate(car_URLs_unique):
        if (ii + 1) % 20 == 0:
            print(f"Processing car {ii+1}", end="\r")
        try:
